...
class FullAdaptiveController(Node):
    def __init__(self):
        ...
        # Desired position initialized from first odometry message.
        self.x_des = None
        self.y_des = None
        self.z_des = None
        self.yaw_des = None
        ...
        # ==========================================================
        # Adaptive arm controller
        # chi_arm = [q1, q2]
        # ==========================================================

        self.joint_names = ["joint1", "joint2"]

        self.Phi_arm = np.diag([
            1.8,   # joint1
            1.2    # joint2
        ])

        self.Lambda_arm = np.diag([
            2.5,   # joint1 torque action
            1.5    # joint2 torque action
        ])

        self.K_hat_arm = np.array([0.01, 0.01, 0.01], dtype=float)
        self.nu_arm = np.array([8.0, 12.0, 12.0], dtype=float)

        self.adaptive_scale_arm = 0.03
        ...
        # ==========================================================
        # Shared adaptive-control parameters
        # ==========================================================
    ...
    def control_loop(self):
        # Drone control starts as soon as odometry is available.
        ...
        # Small-angle mapping:
        # ax approximately controlled by pitch
        # ay approximately controlled by roll
        ...
        if self.have_joint_state and self.q0 is not None:
            ...
            chi_arm = self.q.copy()
            chi_dot_arm = self.q_dot.copy()

            chi_des_arm = q_des.copy()
            chi_dot_des_arm = q_dot_des.copy()

            u_arm, e_arm, s_arm, rho_arm = self.compute_adaptive_control(
                chi=chi_arm,
                chi_dot=chi_dot_arm,
                chi_des=chi_des_arm,
                chi_dot_des=chi_dot_des_arm,
                Phi=self.Phi_arm,
                Lambda=self.Lambda_arm,
                K_hat=self.K_hat_arm,
                nu=self.nu_arm,
                adaptive_scale=self.adaptive_scale_arm,
                dt=dt,
                wrap_yaw_index=None
            )

            tau_arm = np.clip(u_arm, -self.tau_arm_limit, self.tau_arm_limit)
        ...
        if dt_log > 1.0:
            ...
            self.get_logger().info(
                ...
                f"e_arm={np.round(e_arm, 3)} "
                f"s_arm={np.round(s_arm, 3)} "
                f"rho_arm={rho_arm:.3f} "
                f"K_arm={np.round(self.K_hat_arm, 3)} "
