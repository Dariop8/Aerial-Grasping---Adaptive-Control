...
class FullAdaptiveController(Node):
    def __init__(self):
        ...
        # Desired drone position/yaw initialized from first odometry.
        self.x_des = None
        self.y_des = None
        self.z_des = None
        self.yaw_des = None
        ...
        # ==========================================================
        # Unified adaptive controller, closer to paper
        #
        ...
        # u = -Lambda_all s - adaptive_scale_all rho s_direction
        # ==========================================================

        self.Phi_all = np.diag([
            1.40,   # x
            1.40,   # y
            1.40,   # z
            3.5,    # roll
        ...
        self.K_hat_all = np.array([0.01, 0.01, 0.01], dtype=float)
        self.nu_all = np.array([10.0, 14.0, 14.0], dtype=float)
        self.adaptive_scale_all = 0.015

        # ==========================================================
        # Cascaded attitude stabilizer
        #
        # Necessario perché il quadrotore è sottoattuato:
        # x/y non sono comandabili direttamente, ma tramite roll/pitch.
        # Questa parte trasforma ax/ay in roll_des/pitch_des
        # e poi in momenti Mx/My/Mz.
        # ==========================================================
        ...
        # ==========================================================
        # Shared adaptive safety parameters
        # ==========================================================
        ...
        self.get_logger().info("Full unified adaptive controller started")
    ...
    def compute_unified_paper_control(
        self,
        chi,
        chi_dot,
        chi_des,
        ...
        return self.compute_adaptive_control(
            chi=chi,
            chi_dot=chi_dot,
            chi_des=chi_des,
            chi_dot_des=chi_dot_des,
            Phi=self.Phi_all,
            Lambda=self.Lambda_all,
            K_hat=self.K_hat_all,
            nu=self.nu_all,
            adaptive_scale=self.adaptive_scale_all,
            dt=dt,
            wrap_yaw_index=5
        )
    ...
    def control_loop(self):
        ...
        # ======================================================
        # Unified paper-like control
        #
        # chi_all = [x, y, z, roll, pitch, yaw, q1, q2]
        # u_all   = [ax, ay, az, Mx_virtual, My_virtual, Mz_virtual, tau1, tau2]
        # ======================================================
        ...
        u_all, e_all, s_all, rho_all = self.compute_unified_paper_control(
            chi=chi_all,
            chi_dot=chi_dot_all,
            chi_des=chi_des_all,
            chi_dot_des=chi_dot_des_all,
            dt=dt
        )

        ax_cmd = np.clip(u_all[0], -self.ax_limit, self.ax_limit)
        ay_cmd = np.clip(u_all[1], -self.ay_limit, self.ay_limit)
        az_cmd = np.clip(u_all[2], -self.az_limit, self.az_limit)

        # ======================================================
        # Underactuated quadrotor mapping:
        # lateral acceleration -> desired attitude
        # ======================================================
        ...
        # ======================================================
        # Attitude tracking for real motor moments
        # ======================================================
        ...
        # ======================================================
        # Arm torques from unified controller
        # ======================================================
        ...
        if dt_log > 1.0:
            ...
            self.get_logger().info(
                ...
                f"e_all={np.round(e_all, 3)} "
                f"s_all={np.round(s_all, 3)} | "
                f"rho_all={rho_all:.3f} K_all={np.round(self.K_hat_all, 3)} | "
                ...
                f"e_arm={np.round(e_all[6:8], 3)} "
                f"s_arm={np.round(s_all[6:8], 3)} "
